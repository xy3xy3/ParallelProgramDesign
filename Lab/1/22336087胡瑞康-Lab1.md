...
flowchart TD
    subgraph "初始化阶段"
        A[开始] --> B[MPI_Init 初始化环境]
        B --> C[获取进程 rank 和总进程数 size]
        C --> D[进程0: 输入矩阵维度 m,n,k]
        D --> E[MPI_Bcast 广播矩阵维度]
    end

    subgraph "数据分发阶段"
        E --> F[进程0: 初始化矩阵 B]
        F --> G[MPI_Bcast 广播完整矩阵 B 给所有进程]
        G --> H[计算每个进程负责的矩阵 A 的行数]
        H --> I[进程0: 初始化矩阵 A]
        I --> J[MPI_Scatterv 分发矩阵 A 的不同部分给各进程]
    end

    subgraph "计算阶段"
        J --> K[MPI_Barrier 同步所有进程]
        K --> L[开始计时]
        L --> M[每个进程计算本地矩阵乘法]
        M --> N[结束计时]
        N --> O[MPI_Reduce 收集最大计算时间]
    end

    subgraph "结果收集阶段"
        O --> P[MPI_Gatherv 收集各进程计算结果]
        P --> Q[进程0: 输出计算时间]
        Q --> R[释放内存]
        R --> S[MPI_Finalize 结束MPI环境]
        S --> T[结束]
    end

    subgraph "数据分块策略"
        DS[矩阵划分] --> DS1[矩阵A: 行块划分]
        DS --> DS2[矩阵B: 完整广播]
        DS --> DS3[矩阵C: 行块划分]
    end
```
...
这是最常用也是相对直接的方法。可以将大矩阵分割成若干个较小的子矩阵（或称为块）。

假设我们要计算 $C = A \times B$，可以将 $A$ 分割成 $A_{ij}$，将 $B$ 分割成 $B_{jk}$，那么 $C$ 中的每个块 $C_{ik}$ 可以通过以下公式计算：
...
如果矩阵规模非常庞大，甚至连部分子块都无法完全放入内存时，我们需要考虑使用外存（如硬盘）。
